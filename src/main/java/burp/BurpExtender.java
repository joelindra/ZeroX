...
public class BurpExtender
        implements IBurpExtender, IProxyListener, IExtensionStateListener, IHttpListener, IContextMenuFactory, ITab, IMessageEditorController {
    ...
    private IMessageEditor selectedRequestViewer;
    private IMessageEditor selectedResponseViewer;
    private IDORTestResult currentlySelectedAutomateResult;
    ...
    // Real Time IDOR Request/Response viewers (Standard Burp Viewers)
    private IMessageEditor rtOriginalRequestViewer;
    private IMessageEditor rtOriginalResponseViewer;
    private IMessageEditor rtModifiedRequestViewer;
    private IMessageEditor rtModifiedResponseViewer;
    private RealTimeResult currentlySelectedRTResult;
    ...
    private JPanel createRealTimeIDORPanel() {
        ...
        // Results table (Intruder-like columns)
        String[] rtColumnNames = { "#", "Method", "Host", "Path", "Orig Status", "Mod Status", "Orig Len", "Mod Len", "Result" };
        ...
        realTimeResultsTable.setRowHeight(28); // Standard Burp row height
        realTimeResultsTable.setShowHorizontalLines(true);
        realTimeResultsTable.setShowVerticalLines(true); // Intruder shows vertical lines
        ...
        // Column widths
        realTimeResultsTable.getColumnModel().getColumn(0).setPreferredWidth(40); // #
        realTimeResultsTable.getColumnModel().getColumn(1).setPreferredWidth(60); // Method
        realTimeResultsTable.getColumnModel().getColumn(2).setPreferredWidth(120); // Host
        realTimeResultsTable.getColumnModel().getColumn(3).setPreferredWidth(200); // Path
        realTimeResultsTable.getColumnModel().getColumn(4).setPreferredWidth(80); // Orig Status
        realTimeResultsTable.getColumnModel().getColumn(5).setPreferredWidth(80); // Mod Status
        realTimeResultsTable.getColumnModel().getColumn(6).setPreferredWidth(70); // Orig Len
        realTimeResultsTable.getColumnModel().getColumn(7).setPreferredWidth(70); // Mod Len
        realTimeResultsTable.getColumnModel().getColumn(8).setPreferredWidth(90); // Result
    ...
    private JPanel createRTViewerCard(String title, Color accentColor, boolean isOriginal) {
        ...
        if (isOriginal) {
            rtOriginalRequestViewer = callbacks.createMessageEditor(this, false);
            rtOriginalResponseViewer = callbacks.createMessageEditor(this, false);
            tabbedPane.addTab("Request", rtOriginalRequestViewer.getComponent());
            tabbedPane.addTab("Response", rtOriginalResponseViewer.getComponent());
        } else {
            rtModifiedRequestViewer = callbacks.createMessageEditor(this, false);
            rtModifiedResponseViewer = callbacks.createMessageEditor(this, false);
            tabbedPane.addTab("Request", rtModifiedRequestViewer.getComponent());
            tabbedPane.addTab("Response", rtModifiedResponseViewer.getComponent());
        }
    ...
    private void updateRealTimeResultDisplay() {
        ...
        if (selectedRow >= 0 && selectedRow < realTimeResults.size()) {
            ...
            if (result != null) {
                currentlySelectedRTResult = result;
                
                rtOriginalRequestViewer.setMessage(result.originalMessage != null ? result.originalMessage.getRequest() : new byte[0], true);
                rtOriginalResponseViewer.setMessage(result.originalMessage != null ? result.originalMessage.getResponse() : new byte[0], false);
                rtModifiedRequestViewer.setMessage(result.modifiedMessage != null ? result.modifiedMessage.getRequest() : new byte[0], true);
                rtModifiedResponseViewer.setMessage(result.modifiedMessage != null ? result.modifiedMessage.getResponse() : new byte[0], false);
            }
        }
    }
    ...
    private class RealTimeTableCellRenderer extends DefaultTableCellRenderer {
        ...
        public Component getTableCellRendererComponent(JTable table, Object value,
            ...
            if (isSelected) {
                ...
                // Status code columns (4 = Orig Status, 5 = Mod Status)
                if ((column == 4 || column == 5) && value != null) {
                    try {
                ...
                // Length columns (6 = Orig Len, 7 = Mod Len)
                if ((column == 6 || column == 7) && value != null) {
    ...
    private JPanel createViewerCard(String title, Color accentColor, boolean isOriginal) {
        ...
        if (isOriginal) {
            originalRequestViewer = callbacks.createMessageEditor(new AutomateOriginalController(), false);
            originalResponseViewer = callbacks.createMessageEditor(new AutomateOriginalController(), false);
            tabbedPane.addTab("Request", originalRequestViewer.getComponent());
            tabbedPane.addTab("Response", originalResponseViewer.getComponent());
        } else {
            selectedRequestViewer = callbacks.createMessageEditor(new AutomateSelectedController(), false);
            selectedResponseViewer = callbacks.createMessageEditor(new AutomateSelectedController(), false);
            tabbedPane.addTab("Request", selectedRequestViewer.getComponent());
            tabbedPane.addTab("Response", selectedResponseViewer.getComponent());
        }
    ...
    private void updateSelectedResultDisplay() {
        ...
        if (selectedRow >= 0 && selectedRow < currentResults.size()) {
            ...
            if (result != null) {
                ...
                // Update bytes label
                int reqBytes = result.message != null && result.message.getRequest() != null ? result.message.getRequest().length : 0;
                int respBytes = result.message != null && result.message.getResponse() != null ? result.message.getResponse().length : 0;
    ...
    private void displaySelectedResult(IDORTestResult result) {
        if (result != null) {
            currentlySelectedAutomateResult = result;
            selectedRequestViewer.setMessage(result.message != null ? result.message.getRequest() : new byte[0], true);
            selectedResponseViewer.setMessage(result.message != null ? result.message.getResponse() : new byte[0], false);
        }
    }
    ...
    private void clearResults() {
        ...
        selectedRequestViewer.setMessage(new byte[0], true);
        selectedResponseViewer.setMessage(new byte[0], false);
    ...
    private void exportReport() {
        if (currentResults.isEmpty()) {
            JOptionPane.showMessageDialog(mainPanel, "No results to export.", "Export",
        ...
        for (IDORTestResult result : currentResults) {
            report.append("┌─ ").append(result.color.toUpperCase()).append(" [").append(result.riskLevel)
                    .append("] ─────────────────────────────────────\n");
            report.append("│ Status: ").append(result.statusCode).append(" | Length: ").append(result.contentLength)
                    .append(" bytes\n");
            report.append("└────────────────────────────────────────────────────────────\n\n");
    ...
    private void processRealTimeIDOR(IHttpRequestResponse messageInfo) {
        try {
            ...
            // Build modified request with new auth header(s)
            List<String> headers = requestInfo.getHeaders();
            List<String> newHeaders = new ArrayList<>();
            newHeaders.add(headers.get(0)); // Keep the first line (request line)

            // Map to store target headers from user input (name -> full line)
            Map<String, String> targetHeaders = new HashMap<>();
            String[] rawInput = realTimeAuthHeader.split("\n");
            for (String line : rawInput) {
                line = line.trim();
                if (line.contains(":")) {
                    String name = line.split(":", 2)[0].toLowerCase(Locale.getDefault()).trim();
                    targetHeaders.put(name, line);
                }
            }

            Set<String> replacedHeaderNames = new HashSet<>();

            // Iterate through original headers and replace if they match our target headers
            for (int i = 1; i < headers.size(); i++) {
                String originalHeader = headers.get(i);
                if (originalHeader.contains(":")) {
                    String name = originalHeader.split(":", 2)[0].toLowerCase(Locale.getDefault()).trim();
                    if (targetHeaders.containsKey(name)) {
                        newHeaders.add(targetHeaders.get(name));
                        replacedHeaderNames.add(name);
                    } else {
            ...
            // Add any target headers that weren't already in the request
            for (Map.Entry<String, String> entry : targetHeaders.entrySet()) {
                if (!replacedHeaderNames.contains(entry.getKey())) {
                    newHeaders.add(entry.getValue());
                }
            }
            ...
            int modifiedBytes = modifiedResponseObj.getResponse() != null ? modifiedResponseObj.getResponse().length
                    : 0;

            // Get status codes
            IResponseInfo origRespInfo = originalResponse.getResponse() != null
                    ? helpers.analyzeResponse(originalResponse.getResponse())
                    : null;
            IResponseInfo modRespInfo = modifiedResponseObj.getResponse() != null
                    ? helpers.analyzeResponse(modifiedResponseObj.getResponse())
                    : null;
            ...
            // Create result
            String resultStatus;
            if (origStatus == modStatus && originalBytes == modifiedBytes && origBody.equals(modBody)) {
                resultStatus = "NORMAL";
            } else {
            ...
            SwingUtilities.invokeLater(() -> {
                realTimeTableModel.addRow(new Object[] {
                        ...
                        result.modifiedBytes,
                        result.status
                });
                updateRealTimeStats();
            });

            stdout.println("[ZeroX RT] " + result.path + " - Original: " + originalBytes + "B, Modified: " + modifiedBytes
                    + "B -> " + result.status);
        ...
        new Thread(() -> {
            try {
                ...
                SwingUtilities.invokeLater(() -> {
                    ...
                    originalRequestViewer.setMessage(request.getRequest(), true);
                    originalRequestViewer.setMessage(request.getRequest(), false); // This line is redundant, should be originalRequestViewer.setMessage(request.getRequest(), true);
                ...
                byte[] originalResponseBytes = callbacks.makeHttpRequest(httpService, request.getRequest())
                        .getResponse();
                ...
                if (originalResponseBytes != null) {
                    ...
                    SwingUtilities.invokeLater(() -> {
                        originalResponseViewer.setMessage(originalResponseBytes, false);
                        originalBytesLabel
                                .setText(String.format("%,d bytes | %,d bytes", origReqBytesForLabel, origRespBytes));
                    });
                }

                IResponseInfo originalRespInfo = originalResponseBytes != null
                        ? helpers.analyzeResponse(originalResponseBytes)
                        : null;
                ...
                String originalBody = originalResponseBytes != null ? new String(originalResponseBytes).substring(
                ...
                for (Map.Entry<String, String> entry : authHeaders.entrySet()) {
                    ...
                    SwingUtilities.invokeLater(() -> {
                        statusLabel.setText(
                                String.format("⏳ Testing %s... (%d/%d)", color, currentTest, authHeaders.size()));
                    });
                    ...
                    result.message = testResponse; // Store the full message
                    result.requestText = new String(newRequest); // Keep for export
                    result.responseText = responseBytes != null ? new String(responseBytes) : "No response"; // Keep for export
    ...
    private Map<String, String> collectAuthHeaders() {
        ...
        for (Map.Entry<String, String> entry : authHeadersByColor.entrySet()) {
            ...
            if (timestamp != null && (currentTime - timestamp) <= MAX_AUTH_AGE_MS) {
                ...
                long ageMinutes = (currentTime - timestamp) / (60 * 1000);
                stdout.println(
                        "[ZeroX] Skipping expired auth for " + color + " (age: " + ageMinutes + " min, max: 60 min)");
            }
        }
        ...
        if (proxyHistory != null) {
            ...
            for (int i = proxyHistory.length - 1; i >= startIndex; i--) {
                ...
                if (authHeaders.containsKey(color))
                    continue;
    ...
    private static class IDORTestResult {
        ...
        String requestText; // For export
        String responseText; // For export
    }
    ...
    private static class RealTimeResult {
        ...
        IHttpRequestResponse modifiedMessage;

        RealTimeResult(int id, String method, String host, String path, int originalBytes, int modifiedBytes,
                int origStatus, int modStatus, IHttpRequestResponse origMessage, IHttpRequestResponse modMessage,
                String status) {
            this.id = id;
            this.method = method;
            this.host = host;
            ...
            this.originalMessage = origMessage;
            this.modifiedMessage = modMessage;
            this.status = status;
            this.timestamp = System.currentTimeMillis();
        }
    }

    // Controller for Automate BAC Original request/response
    private class AutomateOriginalController implements IMessageEditorController {
        @Override
        public IHttpService getHttpService() {
            return originalRequest != null ? originalRequest.getHttpService() : null;
        }
        @Override
        public byte[] getRequest() {
            return originalRequest != null ? originalRequest.getRequest() : null;
        }
        @Override
    ...
    // Controller for Automate BAC Selected result
    private class AutomateSelectedController implements IMessageEditorController {
        @Override
        public IHttpService getHttpService() {
            return currentlySelectedAutomateResult != null && currentlySelectedAutomateResult.message != null 
                    ? currentlySelectedAutomateResult.message.getHttpService() : null;
        }
        @Override
        public byte[] getRequest() {
            return currentlySelectedAutomateResult != null && currentlySelectedAutomateResult.message != null 
                    ? currentlySelectedAutomateResult.message.getRequest() : null;
        }
        @Override
        public byte[] getResponse() {
            return currentlySelectedAutomateResult != null && currentlySelectedAutomateResult.message != null 
                    ? currentlySelectedAutomateResult.message.getResponse() : null;
        }
    }

    // ==================== IMessageEditorController Implementation for Real Time ====================

    @Override
    public IHttpService getHttpService() {
        if (currentlySelectedRTResult != null && currentlySelectedRTResult.originalMessage != null) {
            return currentlySelectedRTResult.originalMessage.getHttpService();
        }
        return null;
    }
    ...
    public byte[] getRequest() {
        if (currentlySelectedRTResult != null && currentlySelectedRTResult.originalMessage != null) {
            return currentlySelectedRTResult.originalMessage.getRequest();
        }
        return null;
    }
    ...
    public byte[] getResponse() {
        if (currentlySelectedRTResult != null && currentlySelectedRTResult.originalMessage != null) {
            return currentlySelectedRTResult.originalMessage.getResponse();
        }
        return null;
    }
